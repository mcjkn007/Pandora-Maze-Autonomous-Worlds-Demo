...
library GameConfigComponent {
  ...
  function getMetadata() internal pure returns (string memory, string[] memory) {
    ...
    _fieldNames[0] = "config1";
    _fieldNames[1] = "config2";
    return ("GameConfigComponent", _fieldNames);
  }
  ...
  function decode(bytes memory _blob) internal pure returns (GameConfigComponentData memory _table) {
    _table.config1 = (uint256(Bytes.slice32(_blob, 0)));

    _table.config2 = (uint256(Bytes.slice32(_blob, 32)));
  }

  /** Tightly pack full data using this table's schema */
  function encode(uint256 config1, uint256 config2) internal view returns (bytes memory) {
    return abi.encodePacked(config1, config2);
  }
